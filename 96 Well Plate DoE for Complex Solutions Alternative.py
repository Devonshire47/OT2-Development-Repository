...
tiprack_200 = labware.load("opentrons-tiprack-300ul", slot='1')  # Saves a 300ul tiprack to the variable tiprack_200
...
def sort(x, y, z, q):                               # This function determines which is the most accurate pipette to use based on the volume to be dispensed, it also tracks which reagent to use in which well
    ...
    if x == float(0):                               # Checks if the volume to be dispensed is 0
        ...
        P10.distribute(                             # If the volume to be dispensed is less than 30, the P10 is selected for dispensing
            ...
            well_buffers96(y).top(0.5),             # Sets which wells to dispense into, container well_buffers96 position stored in variable y, as well as the height to aspirate from
        ...
        P10.touch_tip(well_buffers96(y))            # With this option declared the pipette tip will be touched to the top of each well after aspirating
        P10.blow_out(well_buffers96(y))             # Sets another blow out of the pipette
        ...
        P300.distribute(                            # Sets the pipette to be used as the P300 instead of the P10
            ...
            well_buffers96(y).top(0.5),             # Sets which wells to dispense to, container well_buffers96 well position stored in y, also sets the height to aspirate from
        ...
        P300.touch_tip(well_buffers96(y))           # With this option declared the pipette tip will be touched to the top of the wells after pipetting
        P300.blow_out(well_buffers96(y))            # assigns another blow out, dispensing a gust of air after dispensing to clear the pipette tip of fluid
